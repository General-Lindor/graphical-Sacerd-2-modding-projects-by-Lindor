    ...
    float4 calcLight(float3 aVertexPosition, float3 aVertexNormal, float3 aCameraPosition, lightData someLightData, out float4 aReturnSpecularColor) {
        ...
        aReturnSpecularColor.xyz = colorize3D(specColor * 0.8f);
        aReturnSpecularColor.w = 0.0f;
        diffColor = colorize3D(diffColor);
    ...
    float4 calcDiffuseLight(float3 aVertexPosition, float3 aVertexNormal, lightData someLightData) {
        ...
        return float4(colorize3D(diffColor), 0.0f);
    }
    ...
    float4 calcItemPreviewColor(float3 tan_to_view0, float3 tan_to_view1, float3 tan_to_view2, float4 tex0, float4 tex1, float4 texNormal) {
        ...
        */
        col = tex0;
            ...
            col = colorize4D(float4(0.2 + 0.5 * saturate(dot(nrm_wrld, normalize(float3(0.5, 0.5, 1.0)))), dot(tex1.xyz, float3(0.222, 0.707, 0.071)), 0.0, 0.0));
        #endif
        return col;
    }
    ...
    float4 apply_colorramp_org(float4 color,float4 mask,float4 ramp[8]) {
            ...
            mask              = mask * 2.0f - 1;
            float4 posConvert = colorize4D(mask);
            float4 negConvert = colorize4D(0.0f - mask);
            ...
            result = colorize4D(lerp(result, color, 1.0f - convertFalseLuminance));
            return result;
        #else
            return color;
        #endif
    ...
    float4 apply_colorramp(float4 color, float4 mask, float4 ramp[8]) {
            ...
            float4 posConvertMul = colorize4D(mask * diffuseLuminance);
            float4 negConvertMul = colorize4D(mask * (0.0f - diffuseLuminance));
