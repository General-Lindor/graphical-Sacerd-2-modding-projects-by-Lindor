...
s2half4 decode_normal(s2half4 value)
{
  ...
  s2half2 enc_normal = (2.0 * value.xy) - float2( 1,1 );
  value = s2half4(enc_normal.x,
                 enc_normal.y,
                 sqrt( 1.0 - dot( enc_normal, enc_normal ) ),
                 value.z);
#elif NORMALFORMAT_88
  value = s2half4(value.x,
                  value.y,
                  sqrt( 1.0 - dot( value.xy, value.xy ) ),
                  1);
#elif NORMALFORMAT_DXN
	value     = (value - s2half4(0.5, 0.5, 5.0, 0))*s2half4(2,2,2,1);
	value.z   = sqrt( 1.0f - value.x*value.x - value.y*value.y )/2.f;
	value.xyz = normalize(value.xyz);  // do we need this?
#elif NORMALFORMAT_DXT5
  value *= s2half4(2,2,2,1);
  value -= s2half4(1,1,1,0);
