...
#define SHADOW_H
  ...
  float readShadowmap(sampler2D shadowMap, float4 smCoord)
  {
    ...
    #else
      float smDepth = tex2Dlod(shadowMap, smCoord);
  ...
  {
	    float res;
  #if defined(SM3_0) && defined(SMOOTH_SHADOWS)
	...
	// make a lot of lookups around our texel to do blur filtering
      ...
      jitterOffset = (2.0 * tex3D(jitterTex, float3(texScale * scrPos.x, texScale * scrPos.y,   i / n))) - 1.0;
			// add
    ...
    smCoord = smPos;
      ...
      // this is it!
      return res;
  #elif defined(SM2_0) || defined(SM3_0)
	    int i;
	    float4 jitterOffset;
