    ...
    uniform float4   vtx_data_array[4])
{
...
#ifdef PROJECT_IN_OBJECT_SPACE
	float4 pos4         = float4(I.position + (bias + glob_z) * I.normal, 1.0);  // let the intensity fade with the height distance around the effect
  ...
  float3 normal       = mul( I.normal, worldMatrix );
         ...
         intensity   *= saturate( 1.0 - ( abs( pos4.z - e_center.z ) - 15.0 ) / 15.0 );
