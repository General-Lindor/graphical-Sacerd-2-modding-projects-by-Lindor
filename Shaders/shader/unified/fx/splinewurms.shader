...
#ifdef REVERSE
  float spline_shift = saturate(param.y * I.data.y + param.w);
#else
	float spline_shift = saturate(-param.y * I.data.y + param.w);
#endif
  float knot_id = knot_scale * spline_shift;
...
#ifdef REVERSE
    O.col0 = tex0 * light_col_amb * sin( I.texcoord0.z * S2_PI );
#else
	  O.col0 = tex0 * light_col_amb;
#endif
