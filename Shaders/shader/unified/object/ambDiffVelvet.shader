...
#include "extractvalues.shader"
  ...
  struct appdata {
        ...
        uniform float4    light_col_diff)
    {
	    ...
	    float3 specular = tex1.xyz * light_col_diff.xyz * pow(saturate(dot(half_vec, sf_nrm)), 10);

      // ambient
      float3 glow_amb = light_col_amb.xyz * tex0.xyz + tex1.a * tex0.xyz;

      float3 final_color = glow_amb + /*shadow.z * */(shiny + specular.xyz); // shadows not working: extreme acne thin polys!
