  ...
  uniform float4   param)
{
  ...
  currentHeightAlpha = (currentHeightAlpha - (pos4.z )) / 1960.0;
  ...
  uniform float4      param)
{
  ...
  float depthscroll = param.x;
	...
	s2half4 tex2 = tex2D(texture1, I.texcoord.xy * 5.0 - depthscroll);
...
O.col[0] = float4(  float3(specular.x * 0.7,specular.x * 0.7,specular.x * 4.0) ,I.v_alpha * specular.x * specular.x * specular.x * specular.x * specular.x);//tex0a * I.v_alpha*(alpha * 5.0 + I.v_glow) / 5.0 * specular.x );
  //glowing rimm on edge to "hard" shield
  if(glowAlpha < 0.01 && glowAlpha > 0.0085)
  {
	glowAlpha = 1.0;
	O.col[1] =  float4(float3(0.2,0.3,1.0) + I.v_glow / 2.0, I.v_alpha * (glowAlpha + I.v_glow) );
  }
  else
	O.col[1] =  float4(float3(specular.x * 0.5,specular.x * 0.5,specular.x * 4.0) + I.v_glow / 1.3, (I.v_alpha * (glowAlpha * 1.3 + I.v_glow) + specular.x / 6.0) * 0.6 );
