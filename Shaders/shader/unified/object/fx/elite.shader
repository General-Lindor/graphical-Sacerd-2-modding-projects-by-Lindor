  ...
  uniform float4      param)
{
  ...
  // out
  float galpha = I.globalAlpha * elapsedTime * mask / 2.0;
  float galphaGlow = (I.globalAlpha + 0.2) * elapsedTime * mask / 2.0;

	O.col[0] = float4(tex0.xyz * galpha ,I.globalAlpha * elapsedTime * mask ); //+ (tex1.xyz * specAlpha)
	O.col[1] = float4(tex0.xyz * galphaGlow * mask,(I.globalAlpha + 0.2) * elapsedTime * mask);
