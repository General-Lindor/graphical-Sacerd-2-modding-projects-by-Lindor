...
#include "extractvalues.shader"
  ...
  struct appdata {
    ...
    //	s2half3 nrm = normalize(tex2.xyz - s2half3(0.5, 0.5, 0.5));
	    ...
	    // calc specular
	    float3 specular = pow(saturate(dot(half_vec, nrm)), 20) * light_col_diff.xyz * tex1.xyz;

	    // calc base color
	    float3 diffuse = saturate(dot(nrm, l_dir)) * light_col_diff * tex0.xyz;

  
      float3 final_color = env_color + shadow.z * (specular + diffuse);
      float3 final_glow  = 0.3 * (shadow.z * specular);
