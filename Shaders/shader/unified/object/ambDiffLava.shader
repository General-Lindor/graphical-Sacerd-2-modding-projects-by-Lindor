  ...
  struct appdata {
    ...
    //	s2half3 nrm = normalize(tex2.xyz - s2half3(0.5, 0.5, 0.5));
      ...
      float3 moon_diff = light_col_amb.xyz * tex0.rgb * (0.5 + saturate(dot(c_dir, nrm)));
	    ...
	    // std color
      float4 std_color = float4(moon_diff + /*shadow.z * */(sun_diff + specular), tex0.a);
      float4 std_glow = float4(0.5 * /*shadow.z * */specular, 0.0);
      ...
      float4 final_color = lerp(std_color, lava_color, tex1.a);
      float4 final_glow  = lerp(std_glow, lava_glow, tex1.a);
