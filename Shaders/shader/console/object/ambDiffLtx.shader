...
#include "extractvalues.shader"
  ...
  struct appdata {
    ...
    #ifdef VS_OUT_vertexLightData
      O.vlColor = computeVertexLightingColor(pos4,nrm4,vtx_lightBlocks);
    #endif 
    ...
    //	s2half3 nrm = normalize(tex2.xyz - s2half3(0.5, 0.5, 0.5));
	    ...
	    // calc specular
	    s2half3 specular = pow(saturate(dot(half_vec, nrm)), 20) * light_col_diff.xyz * tex1.xyz;

	    // calc base color
	    s2half3 diffuse = saturate(dot(nrm, l_dir)) * light_col_diff * tex0.xyz;

  
      s2half3 final_color = env_color + shadow.z * (specular + diffuse);
      s2half3 final_glow  = 0.3 * (shadow.z * specular);
     #ifdef VS_OUT_vertexLightData
        final_color += tex0.rgb*I.vlColor.rgb;
     #endif	    
