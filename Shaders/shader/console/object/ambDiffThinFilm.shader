  ...
  struct appdata {
    ...
    //	s2half3 nrm = normalize(tex2.xyz - s2half3(0.5, 0.5, 0.5));
      ...
      // lighting
	    ...
	    // calc sun diffuse
	    s2half3 sun_diff = light_col_diff.xyz * tex0.xyz * saturate(dot(l_dir, nrm));

      // calc moon diffuse
      s2half moon_diff_f = 0.5 + saturate(dot(c_dir, nrm));
	    s2half3 moon_diff = light_col_amb.xyz * moon_diff_f * tex0.xyz;
        
      // calc glow
      s2half3 glow_amb = tex1.a * tex0;

	    // calc specular
	    s2half spec_factor = pow(saturate(dot(half_vec, nrm)), 20);
	    s2half3 specular = spec_factor * tex1.xyz * light_col_diff.xyz;

	    // calc "oil film depth"
	    s2half fdepth = 0.15 / dot(nrm, c_dir);
	    s2half3 interfCol = spec_factor * tex2.a * light_col_diff.xyz * tex2D(gradient_texture, float2(fdepth, 0.0)).xyz;

      s2half3 final_color = moon_diff + glow_amb + shadow.z * (sun_diff + specular + interfCol);
      s2half3 final_glow  = 0.5 * (glow_amb + shadow.z * specular);
