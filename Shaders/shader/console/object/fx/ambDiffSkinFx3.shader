  ...
  uniform float4    pix_data_array[2] )
{
  ...
  // skin shading
	...
	// interpolate!
	s2half3 magic_col = lerp(tex0.xyz, tex1.xyz, magic_int * pow(sine, 4.0));

	// calc sun diffuse
	s2half3 diffuse = light_col_diff.xyz * magic_col * saturate(dot_l_n);

  // calc moon diffuse
  s2half3 moon_diff = light_col_amb.xyz * magic_col * (0.5 + saturate(dot_c_n));

  // calc glow
  s2half3 glow_amb = tex1.a * tex0 + 0.1 * tex1.xyz * pow(sine, 4.0);
  
	// calc specular
	s2half3 specular = light_col_diff.xyz * tex1.xyz * pow(saturate(dot(half_vec, nrm)), 20);
	// no specular in magic parts!
	specular *= (1.0 - magic_int);
	
	// calc base color
	s2half3 base_color = diffuse + specular + subsurface * tex3.a;

	// prepare output color
	s2half4 out0 = float4(glow_amb + moon_diff + shadow.z * base_color, tex0.a);
	s2half4 out1 = float4(0.5 * shadow.z * specular + glow_amb, tex0.a);
