  ...
  struct appdata {
    ...
    //	s2half3 nrm = normalize(tex2.xyz - s2half3(0.5, 0.5, 0.5));
      ...
      float3 final_color = glow_amb + shadow.z * (0.5 * diffuse +  specular.xyz);
	    ...
	    // compose color
	    half4 swCol = (final_color.x + final_color.z + final_color.y) / 1.4 + float4(0.0,0.0,0.15,0.0);
	    O.col[0].xyz = swCol * stripes.x + noise;
	    O.col[0].a = shimmer;
	    O.col[1] = 0.2;//float4(final_glow, tex0.a);
