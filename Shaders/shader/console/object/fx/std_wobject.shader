  ...
  uniform float4      param)
{
  ...
  // out
	...
	s2half galpha = (I.globalAlpha + I.globalAlpha2) * 0.2;
	O.col[0] = float4(col ,( galpha ) * elapsedTime * (mask) ); //+ (tex1.xyz * specAlpha)
	O.col[1] = float4((tex0.xyz ) * (mask),(galpha ) * elapsedTime * mask);
