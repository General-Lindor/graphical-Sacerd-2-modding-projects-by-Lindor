  ...
  uniform float4      param)
{
  ...
  s2half4 tex1 = tex2D(texture1, I.texcoord.xy + float2(depthscroll * 5.0, depthscroll * 2.0));
  s2half  specAlpha = tex2D(texture1, I.texcoord.xy + float2(depthscroll * -2.0, depthscroll));
  ...
  s2half4 t_mask = tex3D(textureVolume, float3(I.texcoord.xy ,depthscroll * 5.0) + float3(depthscroll, depthscroll ,0.f) ) + float4(0.5,0.5,0.5,0.5);

  // offset'ed background
  s2half4 bgr = tex2D(texture4, lerp(ofs_scr_pos.xy, ofs_scr_pos.zw, t_mask) );
...
//	s2half4 bgr = lerp(nonoffs_bgr, offs_bgr, t_mask.x);
  ...
  // lerp with opacity
  float3 amb = lerp(bgr.xyz, tex0.xyz, saturate(t_mask.x * 0.5 * pulsar + pow(I.texcoord.z, 2.0)));
  ...
  // calc specular
	float3 specular = pow(saturate(dot(half_vec, nrm)), 2) * tex1.xyz * light_col_diff.xyz +  ( t_mask.xyz) / 7.0f;

  // final calc for "build-up"/"build-down" (is via alpha!)
  float alpha = step(saturate(I.texcoord.y), elapsedTime);

  // out
//	O.col[0] = float4(specular + amb  ,alpha * tex1.x * specAlpha ); //+ (tex1.xyz * specAlpha)
	O.col[0] = float4(amb + (specular * specAlpha * tex1.x),alpha );
	O.col[1] = float4(tex1.xyz* specAlpha, tex1.x* specAlpha);
