...
#include "extractvalues.shader"
  ...
  struct appdata {
	...
	#ifdef CONSOLE_IMPL
		tex2.z /= 2.f;
	  tex2.xyz = normalize(tex2.xyz);
	#endif	
	    // get out of half-space to -1..1
    //	s2half3 nrm = normalize(tex2.xyz - s2half3(0.5, 0.5, 0.5));
	    s2half3 nrm = tex2.xyz;
	    // need another "abgeschwächte" normal! Abschwächung comes from normalmap alpha
...
#ifdef XENON_IMPL       
      float4 scr_temp = float4( tiling_data_half_tile.zw*vPos.xy,
                               tiling_data_half_tile.zw*(vPos.xy + 2500*nrm.xy/surfaceZ) );
#else
      float4 scr_temp = float4( target_data.zw*vPos.xy,
                               target_data.zw*(vPos.xy + 2500*nrm.xy/surfaceZ) );
#endif                               
      float2 scr_pos = scr_temp.xy;
      float2 offs_scr_pos = scr_temp.zw;
      ...
      s2half3 fin_color = env_color + final_color + specular;
      s2half3 final_glow  = specular;

    #ifdef S2_FOG
      float2 fog_tmp = { I.lightDist.w, I.camDist_ts.w };
      fogDiffuse( fin_color, fog_texture, fog_tmp, light_col_diff );
