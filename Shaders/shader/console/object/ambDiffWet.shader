...
#include "extractvalues.shader"
  ...
  struct appdata {
    ...
    //	s2half3 nrm = normalize(tex2.xyz - s2half3(0.5, 0.5, 0.5));
      ...
      // calc wet factor
	    s2half3 wet_specular = tex2.a * smoothstep(0.85, 0.95, saturate(dot(half_vec, nrm))) * light_col_diff.xyz;

      s2half3 final_color = glow_amb + moon_diff + shadow.z * (sun_diff + specular + wet_specular);
      s2half3 final_glow  = 0.5 * shadow.z * (specular + 0.1 * wet_specular) + glow_amb;
