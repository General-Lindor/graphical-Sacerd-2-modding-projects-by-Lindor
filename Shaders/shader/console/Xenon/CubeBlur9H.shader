...
PASSTHRU_VERTEX mainVS( float4 vPosition : POSITION )
{
    ...
    Output.TexOffset = mul( g_fOffset, g_CubeRot );
    return Output;
}
...
struct fragout {
				  ...
				  in float3 vTexOffset : TEXCOORD1 )
  {
      ...
      float4 z0, z1, z01;
      z0.x   = dot( t0.rb, float2(255.0/256, 1.0/256) );
      z1.x   = dot( t0.ga, float2(255.0/256, 1.0/256) );
      z0.y   = dot( t1.rb, float2(255.0/256, 1.0/256) );
      z1.y   = dot( t1.ga, float2(255.0/256, 1.0/256) );
      z0.z   = dot( t2.rb, float2(255.0/256, 1.0/256) );
      z1.z   = dot( t2.ga, float2(255.0/256, 1.0/256) );
      z0.w   = dot( t3.rb, float2(255.0/256, 1.0/256) );
      z1.w   = dot( t3.ga, float2(255.0/256, 1.0/256) );
      
      z01.r = dot( t4.rb, float2(255.0/256, 1.0/256) );
      z01.g = dot( t4.ga, float2(255.0/256, 1.0/256) );
