 ...
 ********************************************************************/
     ...
     *************************************************************************/
#if defined(SM3_0)&&!defined(PS3_IMPL)
...
#ifdef PS3_IMPL
  VSO.toLight = mul( mat_TS, light_pos.xyz ); //TB: COLOR1 saturates
#else
  VSO.toLight = mul( light_pos.xyz, mat_TS ); //TB: COLOR1 saturates
#endif
  
    // Output vector to camera
#ifdef MINIMAPMODE
    VSO.toCamera = normalize( -camera_pos.xyz );
#else
#ifdef PS3_IMPL
	  VSO.toCamera = mul( mat_TS, normalize( camera_pos.xyz - pos_tmp )); //TB: COLOR0 saturates
#else
	  VSO.toCamera = mul( normalize( camera_pos.xyz - pos_tmp ), mat_TS ); //TB: COLOR0 saturates
#endif
#endif
                ...
                )
{
  ...
  s2half3 watercolor  = pix_data_array[PS_WATER_COLOR].rgb;
  s2half  depthfade   = pix_data_array[PS_WATER_COLOR].a;  
  s2half2 fresnel_lim = pix_data_array[PS_WATER_PARAMETERS].xy;
  s2half  spec_pow    = pix_data_array[PS_WATER_PARAMETERS].z;
  s2half  refr_str    = pix_data_array[PS_WATER_PARAMETERS].w;
  s2half2 foam_depth  = pix_data_array[PS_FOAM_PARAMETERS].xy;
  s2half2 shore_trans = pix_data_array[PS_FOAM_PARAMETERS].zw; // x is min transparency, y is depth scale
  // map interpolators
...
#else
  float3 eyeVec   = normalize( In.toCamera );
...
#ifdef RAINING
  // Read rain texture
  s2half3 looky3d  = { In.rainCoords.xy, In.params.x * 0.7 };
  s2half4 rain     = tex3D( textureVolume, looky3d );
  s2half3 rainnorm = rain.xyz * 2.0 - 1.0;
  //norm = normalize( rainnorm + norm );
...
*/
  ...
  mat_TS[0].z  = sqrt( 1.0 - dot( In.base_TS.xy, In.base_TS.xy ) );
  mat_TS[1].xy = In.base_TS.zw;
  mat_TS[1].z  = sqrt( 1.0 - dot( In.base_TS.zw, In.base_TS.zw ) );
  mat_TS[2]    = cross( mat_TS[0], mat_TS[1] );

  // calculate vectors for environment lookup
#ifdef PS3_IMPL
  float3 norm_WS = mul( mat_TS, norm );
  float3 eye_WS  = mul( mat_TS, eyeVec );
#else
...
#ifdef CONSOLE_IMPL
  #ifdef XENON_IMPL  
    float4 refract_tcs = float4( tiling_data_half_tile.zw*(vPos.xy                          ),
                                 tiling_data_half_tile.zw*(vPos.xy + norm_WS.xy * refr_str) );
  #else
    float4 refract_tcs = float4( target_data.zw*(vPos.xy                          ),
                                 target_data.zw*(vPos.xy + norm_WS.xy * refr_str) );
  #endif
#endif
...
#endif
#ifdef CONSOLE_IMPL
  float mask = DEPTH_SAMPLE_RAW(depth_map, refract_tcs.zw).x;
  mask = (2*Z_FAR*Z_NEAR) / ((Z_FAR+Z_NEAR) - (2*mask-1)*(Z_FAR-Z_NEAR));
  refract_tcs.xy = (mask<viewdepth) ? refract_tcs.xy : refract_tcs.zw;
#else         
  float4 mask            = tex2Dproj( texture1, refract_tcs );
   ...
   s2half lightning    = step( 0.4, dot( norm_WS, lightning_WS ) ) * param.w;
#else
  float3 environment = texCUBE( textureCube, reflect( - eyeVec, norm ) ).rgb;
  s2half lightning   = 0.0;
#endif

#ifdef CONSOLE_IMPL
  // screenpos of this pixel, zw is refracted
  s2half4 background  = tex2D( texture0, refract_tcs.xy );
  float   grounddepth = DEPTH_SAMPLE_RAW( depth_map, refract_tcs.xy ).x;
  ...
  #ifdef CONSOLE_IMPL  
    float  weight_soft = saturate(waterdepth)*pow( cos( saturate( depth / foam_depth.x ) * S2_PI * 0.5 ), 1.0 );
    float  weight_hard = saturate(waterdepth)*pow( cos( saturate( depth / foam_depth.y ) * S2_PI * 0.5 ), 1.0 );
  #else
...
//  s2half3 abovewater = (specular * 5.0 + environment) * light_col_diff.rgb;
  s2half3 abovewater = ( specular * 5.0 ) * light_col_diff.rgb + environment;
  
  // compose final color
#ifdef ALPHA_BLEND_FOAM
  s2half3 result = lerp( underwater, abovewater, fresnel );
         result = lerp( result, foam.rgb * light_col_diff.rgb, foam.a ) + crest * light_col_diff.rgb;
#else
  s2half3 result = lerp( underwater, abovewater, fresnel ) + ( foam.rgb + crest ) * light_col_diff.rgb;
#endif
  ...
  // fog
    ...
    fogGlowMKSimple( result, fog_texture, fog_coord, light_col_diff );
  #else
