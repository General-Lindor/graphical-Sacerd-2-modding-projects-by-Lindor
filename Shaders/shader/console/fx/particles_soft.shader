...
//  float4 final_col = (I.color + I.color.a * I.brightness) * tex0;
  ...
  s2half4 final_col = (I.color * float4(2,2,2,1)) * tex0;
#ifdef STREAK_SHADER  
  final_col.rgb *= I.color.a;
#endif

  s2half4 blended = s2half4(final_col.rgb * final_col.a, final_col.a);
  s2half4 added = s2half4(final_col.rgb, 0 );

  //set_out_color( float4( 1,0,0,1 ) );
  set_out_color( I.toBlend.x < 0.5? blended:added );
