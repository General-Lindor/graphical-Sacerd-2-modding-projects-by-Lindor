...
//  pos4 *= float4(extrusion ,height, 1.0, 1.0);
  pos4 *= float4(height , height, extrusion  ,1.0);
...
//refraction
	...
	s2half4 bgr = tex2D(texture4, lerp(ofs_scr_pos.xy, ofs_scr_pos.zw, ((1.0 - alpha) * 0.8 + 0.2) * fade) );
...
//
//	// lerp with mask
//	s2half4 bgr = lerp(nonoffs_bgr, offs_bgr  , ((1.0 - alpha) * 0.8 + 0.2) * fade);

  // lerp with opacity
  float3 amb = lerp(bgr.xyz, tex0.xyz,  0.05f * 1.0);

	s2half4 texGate = tex2D(texture6, I.texcoord0.xy) * fade;

	// compose color & glow
//	O.col[0] = float4(amb, 1.0);
//	O.col[0] = float4(1.0,1.0,1.0, 1.0);

	O.col[0] = float4(amb, justAlpha * fade);
	O.col[1] = float4((0.4 * perlin_col.xyz * ob_col2.xyz + 0.3 * ob_col.xyz) * I.decalData.z, alpha * fade);
	O.col[1].xyz *= alpha * fade;
	
	O.col[1] = O.col[1] + texGate;
