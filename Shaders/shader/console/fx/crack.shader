...
struct pixdata {
    ...
    uniform float4      system_data)
  {
    ...
    float depth = DEPTH_SAMPLE(shadow_texture, scr_pos).x;
    float current_delta = 0.5;//abs(depth - surfaceZ);
#else    
	float current_delta = abs(getDepth(tex2Dproj(shadow_texture, I.screenCoord).xy) - I.texcoord.w);
#endif	
    ...
    // out
	  ...
	  O.col[1] = float4( I.groundPos.w * fader_factor * horizon_factor * pl * height_color.xyz, 0.0);
