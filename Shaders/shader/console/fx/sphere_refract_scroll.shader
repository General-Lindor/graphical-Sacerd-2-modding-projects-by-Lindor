  ...
  )
{
  ...
  pos4 *= float4(1.0 , extrusion,1.0, 1.0);
  pos4 *= float4(height , 1.0, height, 1.0);
  ...
  )
{
	...
	float2 texcoord = I.texcoord0.xy + float2( param.y, param.y / 2.0);

	// diffuse color & opacity from texture0
	s2half4 tex0 = tex2D(texture0, texcoord + float2(0, param.y));
	s2half4 tex0Noshift = tex2D(texture0, I.texcoord0.xy);
	// specular color from texture1
	s2half4 tex1 = tex2D(texture1, texcoord);
	
	// refraction offset from bump-map
	s2half4 tex2 = tex2D(texture2, -texcoord);
	s2half4 alpha = tex2D(texture2,-2.0 * texcoord);
	...
	s2half4 bgr = tex2D(texture4, lerp(ofs_scr_pos.xy, ofs_scr_pos.zw + tex0Noshift, fader * alpha.a) );
...
//	
//	// lerp with mask
//	s2half4 bgr = lerp(nonoffs_bgr, offs_bgr + tex0Noshift , fader * alpha.a);

  // lerp with opacity
  float3 amb = lerp(bgr.xyz, tex0.xyz,  0.05f * fader);
  
//  amb = amb + tex0.xyz / 2.0 * fader;

  // out
//	O.col[0] = float4(tex2.xyz , 1.0);
	O.col[0] = float4(amb , alpha.a * tex0Noshift.a);
