                ...
                uniform float4    vtx_data_array[4]    )
{
  ...
  float  eff_height   = vtx_data_array[1].w;            // overall height of the entire effect
...
#else
#ifdef XENON_IMPL
  zoffset            += vtx_data_array[3].w * layer_dist;
  float instance      = vtx_data_array[3].w / anz_layer;
  // intensity for the pixel shader
  // we only want areas within a certain height range around the symbol to show the effect
  intensity          *= saturate( 1.0 - ( abs( pos4.z - e_center.z ) - 50.0 ) / 50.0 );
  O.data              = intensity * saturate(3.0 * (1.0 - instance) / anz_layer + 0.35 * pow(1.0 - instance, 30.0));
#else
  // we only want areas within a certain height range around the symbol to show the effect
  intensity          *= saturate( 1.0 - ( pos4.z - e_center.z - 50.0 ) / 50.0 );
  ...
  O.tcs[0].xy = ttc;
  O.tcs[0].zw = 0.2 * ttc - color_offset;
#endif
  ...
  // write to output register
  O.tcs[1].xy = ttc;
  O.tcs[1].zw = 0.2 * ttc - color_offset;
#endif
  ...
  // write to output register
  O.tcs[2].xy = ttc;
  O.tcs[2].zw = 0.2 * ttc - color_offset;
#endif
  ...
  // write to output register
  O.tcs[3].xy = ttc;
  O.tcs[3].zw = 0.2 * ttc - color_offset;
#endif
  ...
  // write to output register
  O.tcs[4].xy = ttc;
  O.tcs[4].zw = 0.2 * ttc - color_offset;
#endif
  ...
  // write to output register
  O.tcs[5].xy = ttc;
  O.tcs[5].zw = 0.2 * ttc - color_offset;
#endif
  ...
  // write to output register
  O.tcs[6].xy = ttc;
  O.tcs[6].zw = 0.2 * ttc - color_offset;
#endif
  ...
  // write to output register
  O.tcs[7].xy = ttc;
  O.tcs[7].zw = 0.2 * ttc - color_offset;
#endif
  ...
  // write to output register
  O.tcs[8].xy = ttc;
  O.tcs[8].zw = 0.2 * ttc - color_offset;
#endif
