...
#ifdef PS3_IMPL
  float2x2 mat   = float2x2( vtx_data_array[7].xz, vtx_data_array[7].yw );
#else
  float2x2 mat   = vtx_data_array[7];
#endif
           tuv   = mul( tuv, mat );

  // Special case for dealing with the hero -> GeForce 8xxx cards don't need this, but GeForce 7xxx cards do.
  // The division by the zoom factor must happen within the braces, otherwise GF7 cards seem to divide by
  // zero if tuv is zero for whatever reason
  if( abs( tuv.x ) > 0.0001 || abs( tuv.y ) > 0.0001 )
  {
             ...
             uniform float4    param )
{
  ...
  // circular rim
  float4 rim_color_circ = float4( 240.0 / 255.0, 222.0 / 255.0, 160.0/ 255.0, 0.0 );
  ...
  // rectangular rim
  float4 rim_color_rect = float4( 240.0 / 255.0, 222.0 / 255.0, 160.0/ 255.0, 0.0 );
...
void mainPS( in float4 tcs   : TEXCOORD0,
            ...
            uniform sampler2D texture3,   //MK fog of war map
            uniform sampler1D texture4 )  //MK fog of war border gradient map
{
...
//MK fog of war
  float3 fow    = tex2D(texture3, uv.xy).rgb;
  float  fowed  = dot(color, float3(.3,.59,.11));              //MK get luminance of worldmap
  float3 recolor= tex1D(texture4, fowed).rgb;                  //MK get adjustment color based on fow intensity
  
  fow = fow-.5*2;

// ease in/out
  float filter_delta = ((fow*fow)/2+.5) / 1024.0 ;

// linear
//	float filter_delta = (1.0 ) / 1024.0 ;
  ...
  color.rgb = lerp( color.rgb, (color.rgb * 0.2f + float3(fowed, fowed, fowed) * 0.5f) , fow );
  float  fade   = param.x;
